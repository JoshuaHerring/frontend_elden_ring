...
// Garbage Test - Don't understand API
async function searchRenderBosses(){
    const bosses = await getBosses();
    ...
    bosses.forEach(boss => {
        ...
        for (let key in boss) {
            ...
            // if the key is the name make it a header add the boss name to the html and give it a class then add it to the ul holder
                ...
                // Just gets rid of stupid boss name at the moment and literally nothing else
                const buttonSearch = document.getElementById("searchButton")
                buttonSearch.addEventListener("click", () => {
                    let bossName = parseInt(document.getElementById('searchBar').innerHTML)
                    if (bossName == "Name")
                    {
                        let bossTrait = document.createElement("h2");
                        bossTrait.innerHTML = boss[key];
